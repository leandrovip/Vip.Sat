...
using Vip.Sat.Enum;
using Vip.Sat.Events;
using Vip.Sat.Exceptions;
using Vip.Sat.Extensions;
using Vip.Sat.Response;
using Vip.Sat.Settings;

namespace Vip.Sat.Service
{
    /// <summary>
        ...
        private T FinalizaComando<T>(string resposta) where T : SatResposta
        {
            ...
            if (resp.NumeroSessao != Sessao)
                if (Configuracoes.ValidarNumeroSessaoResposta)
                {
                    ...
                    {
                        consultaCount++;
                        Guard.Against<VipException>(consultaCount > Configuracoes.NumeroTentativasValidarSessao,
