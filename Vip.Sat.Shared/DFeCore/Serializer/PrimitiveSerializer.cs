...
namespace Vip.Sat.DFeCore.Serializer
{
    internal static class PrimitiveSerializer
    {
        ...
        private static XObject ProcessContent(DFeBaseAttribute tag, string conteudoProcessado, bool estaVazio, SerializerOptions options)
        {
            ...
            if (!string.IsNullOrEmpty(conteudoProcessado.Trim()) && conteudoProcessado.Length > tag.Max) alerta = DFeSerializer.ErrMsgMaior;

            if (!string.IsNullOrEmpty(alerta.Trim()) && DFeSerializer.ErrMsgVazio.Equals(alerta) && !estaVazio) alerta += $" [{tag.Name}]";
